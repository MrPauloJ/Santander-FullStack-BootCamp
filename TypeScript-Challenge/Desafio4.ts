...
searchButton.addEventListener('click', async () => {
  ...
  // Corrigir sintaxe de elemento HTML
...
// Corrigir sintaxe elementos html
function preencherSenha() {
...
async function criarRequestToken ():Promise<void> {
  let result = JSON.stringify(await HttpClient.get({
    url: `https://api.themoviedb.org/3/authentication/token/new?api_key=${apiKey}`,
    method: "GET"
  }))
  requestToken = JSON.parse(result).request_token
}
...
async function criarSessao() {
  ...
  sessionId = JSON.parse(result).session_id;
}
